...
class BatchQueue():
    ...
    def stop(self):
        ...
        # self.cv.notify()
        # self.cv.release()
        print("[LOG] batch_q.stop() called")
        self.running = False
        with self.cv:
            print("[LOG] batch_q.stop(): Notifying all waiting threads")
            self.cv.notify_all()
    ...
    def wait_batch(self, batch_id: int):
        ...
        while True:
            if batch_id <= self.highest_ready_batch_id:
                # print(f"[BATCH_QUEUE] batch_id {batch_id} is ready (<= {self.highest_ready_batch_id})")
                break
            elif not self.running:
                # print(f"[BATCH_QUEUE] stop() detected. breaking wait loop (batch_id={batch_id})")
                break
            else:
                # print(f"[BATCH_QUEUE] waiting for batch_id {batch_id} to become ready...")
                self.cv.wait()
                continue
        self.cv.release()
