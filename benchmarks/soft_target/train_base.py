...
def setup_distributed():
    dist.init_process_group(backend="nccl")  # Best for multi-GPU training
    local_rank = int(os.environ["LOCAL_RANK"])
    world_size = dist.get_world_size()
    torch.cuda.set_device(local_rank)  # Assign correct GPU
...
def main():
    ...
    # Move model to correct GPU before wrapping it in DDP
    ...
    train_sampler = DistributedSampler(train_dataset(root=args.img_root, transform=train_transform))
    train_loader = torch.utils.data.DataLoader(
        train_dataset(root=args.img_root, transform=train_transform),
    ...
    test_sampler = DistributedSampler(test_dataset(root=args.img_root, transform=test_transform), shuffle=False)
    test_loader = torch.utils.data.DataLoader(
        test_dataset(root=args.img_root, transform=test_transform),
    ...
    best_top1 = 0
    if args.epochs == 0:
        print(f"[rank{local_rank}]: Exiting without training (epochs=0)")
