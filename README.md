...
이 저장소는 다음 평가 항목을 기준으로 정리되어 있습니다.

- 프로젝트 설명: 본 README의 `프로젝트 설명` 섹션에 연구 목적과 핵심 기능을 기술했습니다.
- Source code 설명: `Source Code 설명` 섹션에서 주요 디렉터리와 핵심 모듈 역할을 설명했습니다.
- How to build: 별도 컴파일이 필요 없는 Python 연구 코드이므로, 환경 구성과 import/compile 검증 방법을 `How to Build`에 기술했습니다.
- How to install: Conda 환경 생성, Python package 설치, CUDA/NCCL 관련 시스템 설정을 `How to Install`에 기술했습니다.
- How to test: GPU 없이 가능한 regression test와 실제 GPU/데이터가 필요한 E2E test를 분리해 `How to Test`에 기술했습니다.
- Sample data 설명: 저장소에 포함된 class list, planner profile, sample logs, summary JSON/JSONL, figure 산출물을 설명했습니다.
- Database or data used: 별도 DB를 사용하지 않으며, ImageNet/ImageNet100 파일 시스템 기반 데이터를 사용한다는 점을 명시했습니다.
- Used open source 설명: TSPipe, PyTorch, TorchVision, timm, transformers 등 사용한 주요 오픈소스를 명시했습니다.

### 업로드 포함 여부

GitHub에 반드시 올라가야 하는 항목:

- `tspipe/`: TSPipe runtime, worker, scheduler, profiler, failover 확장 코드
- `benchmarks/soft_target/`: KD 학습 entry point, 모델, loss, planner, YAML 설정
- `benchmarks/soft_target/planner/profile/snet.csv`
- `benchmarks/soft_target/planner/profile/tnet.csv`
- `dataset/`: ImageNet100 loader와 class list
- `scripts/`: plotting, summarization, background load, 분석 helper
- `run_e2e_failover.sh`, `e2e_failover_batch*.sh`, `e2e_tspipe_batch*.sh`
- `run_failover_experiment.py`, `run_experiment0_checkpoint.py`, `run_experiment0b_failover.py`
- `test_minimal_failover.py`, `test_failover_restart_monitoring.py`, `test_failover_logic.py`
- `alpha_beta_values.json`, `benchmarks/soft_target/alpha_beta_values.json`, `measured_restart_costs.json`
- `README.md`, `README_FAILOVER.md`, `TSPipe_Failover_Implementation_Guide.md`, `FAILOVER_ARCHITECTURE_ANALYSIS.md`, `FLOWCHART_VALIDATION_REPORT.md`
- 기존에 Git이 추적하고 있는 sample result/log/figure 파일

GitHub에 올리지 않아도 되는 항목:

- 원본 ImageNet 전체 이미지 데이터셋
- 새로 생성되는 대용량 checkpoint (`*.pt`, `*.pth`, `*.pth.tar`, `*.ckpt`)
- 새로 생성되는 `results/`, `runs/`, TensorBoard log, profiler database, `.venv/`

주의: `benchmarks/soft_target/planner/profile/snet.csv`와 `tnet.csv`는 일반 CSV 로그가 아니라 planner 실행에 필요한 필수 경량 프로파일 데이터입니다. `.gitignore`에서 이 두 파일이 업로드될 수 있도록 예외 처리했습니다.
...
ImageNet100 실험은 원본 ImageNet 이미지가 저장소에 포함되어 있지 않으므로, 로컬 ImageNet 경로를 준비해야 합니다.
...
torchrun --nproc_per_node=4 train_base.py   --img_root "$IMG_ROOT"   --save_root ./results/base/   --epochs 20   --data_name imagenet100   --net_name vit_large   --num_class 100   --batch_size 128   --note base-i100-vit-large
```
...
```bash
cd benchmarks/soft_target
torchrun --nproc_per_node=4 train_base.py   --img_root "$IMG_ROOT"   --save_root ./results/base/   --epochs 0   --data_name imagenet100   --net_name resnet152   --num_class 100   --batch_size 128   --note base-i100-resnet152
```
...
bash ./run_e2e_failover.sh   --img_root "$IMG_ROOT"   --data_name imagenet100   --t_name vit_large   --s_name resnet152   --kd_mode st   --lambda_kd 0.1   --T 4.0   --t_model /path/to/teacher/model_best.pth.tar   --s_init /path/to/student/initial_r152.pth.tar   --batch_size 64   --num_class 100   --epochs 1   --max-steps-per-epoch 50   --tspipe-enable   --tspipe-config benchmarks/soft_target/tspipe.yaml
```
...
bash ./run_e2e_failover.sh   --img_root "$IMG_ROOT"   --data_name imagenet100   --t_name vit_large   --s_name resnet152   --kd_mode st   --lambda_kd 0.1   --T 4.0   --t_model /path/to/teacher/model_best.pth.tar   --s_init /path/to/student/initial_r152.pth.tar   --batch_size 64   --num_class 100   --epochs 1   --max-steps-per-epoch 100   --tspipe-enable   --tspipe-config benchmarks/soft_target/tspipe.yaml   --inject-slowdown-gpu 0   --slowdown-task-scope compute   --slowdown-mode fixed   --slowdown-fixed-ms 800   --slowdown-start 20   --slowdown-end 100
```
...
bash ./run_e2e_failover.sh   --img_root "$IMG_ROOT"   --data_name imagenet100   --t_name vit_large   --s_name resnet152   --kd_mode st   --lambda_kd 0.1   --t_model /path/to/teacher/model_best.pth.tar   --s_init /path/to/student/initial.pth.tar   --batch_size 8   --num_class 100   --epochs 1   --max-steps-per-epoch 5   --tspipe-enable   --tspipe-config benchmarks/soft_target/tspipe.yaml
```
...
실제 학습에 필요한 원본 ImageNet 데이터와 대형 teacher/student checkpoint는 저장소에 기본 포함하지 않는 것이 맞습니다. 대신 README의 실행 예시처럼 `--img_root`, `--t_model`, `--s_init` 인자로 외부 경로를 지정합니다.
...
현재 로컬 저장소는 이미 Git metadata를 가지고 있습니다. 다만 기존 remote가 목표 저장소와 다르므로 아래 순서로 연결하면 됩니다.

현재 상태 확인:

```bash
git config --global --add safe.directory /acpl-ssd10/Synapse-0325
git status --short --branch
git remote -v
```

현재 확인된 remote는 다음과 같습니다.

```text
mine   https://github.com/h2oung/Synapse-checkpoint.git
origin https://github.com/wonjungm/Synapse-0323.git
```

기존 `origin`을 보존하고 새 목표 저장소를 `origin`으로 쓰려면:

```bash
git remote rename origin origin-0323
git remote add origin https://github.com/wonjungm/Synapse.git
git remote -v
```

기존 `origin`을 그냥 바꿔도 된다면:

```bash
git remote set-url origin https://github.com/wonjungm/Synapse.git
git remote -v
```

README와 필수 planner CSV를 staging:

```bash
git add README.md .gitignore
git add -f benchmarks/soft_target/planner/profile/snet.csv
git add -f benchmarks/soft_target/planner/profile/tnet.csv
git status --short
```
...
```bash
git commit -m "docs: expand README for reproducibility"
```
...
만약 GitHub 원격 저장소에 이미 초기 README commit이 있어서 `non-fast-forward` 또는 unrelated histories 오류가 나면 두 가지 선택지가 있습니다.
...
git status
git add README.md .gitignore benchmarks/soft_target/planner/profile/snet.csv benchmarks/soft_target/planner/profile/tnet.csv
git commit
git push -u origin main
```

현재 로컬 내용을 기준으로 원격 branch를 교체:

```bash
git push -u origin main --force-with-lease
```

`--force-with-lease`는 원격 저장소의 기존 내용을 덮어쓸 수 있으므로, 팀에서 이 로컬 폴더를 최종 기준으로 삼기로 합의했을 때만 사용하세요.
