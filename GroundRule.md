# Team Synapse: Ground Rules (v2.0)
...
1.  **메인 채널 (Main Channel):** 모든 논의는 `[노션/슬랙/카카오톡]` 채널을 통해 투명하게 공유하는 것을 원칙으로 합니다.
   - Notion: 논문 세미나 정리, 실험 설계서, 최종 리포트 
   - Slack/Kakao: 실시간 에러 로그, 간단한 아이디어 브레인스토밍
2.  **정기 회의 (Regular Meetings):** 주 1회 정기 회의를 진행하여 진행 상황을 공유하고 이슈를 논의합니다. 
3. **교수님 미팅 (Advisor Meeting)**  
   - 1주 간격으로 지도교수님과 미팅을 진행합니다.
...
5.  **응답 (Response):** 긴급한 사안이 아닐 경우, 메시지 확인 후 12시간 이내에 응답하는 것을 목표로 합니다. 오후 11시 이후 긴급하지 않은 메시지는 다음 날 오전 9시 이후 확인 및 답변하도록 합니다. 
...
1.  **버전 관리 (Version Control):**
    ...
    - PR 생성 시 구현 기능, 테스트 결과(캡처본 등), 변경 사항을 요약하여 작성하며, 리뷰어는 로직의 오류뿐만 아니라 성능 최적화 관점에서도 피드백을 공유합니다. 
